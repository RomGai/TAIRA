...
def _rewrite_pseudo_query_with_llm(
    llm: Qwen3RouterLLM | None,
    user_query: str,
    item_id: str,
    title: str,
    desc: str,
...
def _adaptive_embedding_fusion(
    ...
    enable_llm_recall_control: bool = False,
    llm_model_name: str = "Qwen/Qwen3-8B",
    enable_llm_pseudo_query_rewrite: bool = False,
    user_query: str = "",
) -> Tuple[List[str], Dict[str, Any]]:
    max_available = max(1, len(filtered_item_ids))
    min_recall = int(max(1, min(max_available, min_total_recall)))
    max_recall = int(max(min_recall, min(max_available, max_total_recall)))
    ...
    for step, iid in enumerate(pseudo_targets, start=1):
        ...
        estimated_k = _estimate_total_k(memory, int(max_recall))
        if recall_llm is not None:
            llm_update = recall_llm.optimize_modulation_params(
                query="",
                step=step,
                target_item_id=iid,
                pseudo_query=pseudo_query_map.get(iid, ""),
                text_rank=int(text_rank),
                vl_rank=int(vl_rank),
                current_text_weight=float(text_weight),
                current_vl_weight=float(vl_weight),
                current_total_recall=int(total_k),
                rule_hints={
                    "rule_next_text_weight": float(text_weight),
                    "rule_next_vl_weight": float(vl_weight),
                    "rule_next_total_recall": int(estimated_k),
                },
                recent_memory=memory,
                min_total_recall=int(min_recall),
                max_total_recall=int(max_recall),
            )
            if isinstance(llm_update, dict):
                total_k = int(max(min_recall, min(max_recall, int(llm_update.get("total_recall", estimated_k)))))
                memory[-1]["llm_recall_decision"] = int(total_k)
                memory[-1]["llm_recall_reasoning"] = str(llm_update.get("reasoning", ""))
            else:
                total_k = int(max(min_recall, min(max_recall, estimated_k)))
        else:
            max_step_up = max(20, int(total_k * 0.25))
            max_step_down = max(20, int(total_k * 0.2))
            bounded_k = max(total_k - max_step_down, min(total_k + max_step_up, estimated_k))
            total_k = int(max(min_recall, min(max_recall, bounded_k)))
        memory[-1]["estimated_total_recall_raw"] = int(estimated_k)
    ...
    fused = _merge_unique_ids(text_ids, vl_ids)[: min(max_available, total_k)]
    return fused, {
        "enabled": True,
        ...
        "total_recall": int(min(max_available, total_k)),
    ...
    for row_idx, row in query_df.iterrows():
        ...
        if bool(getattr(args, "enable_agent3_adaptive_weighting", False)):
            adaptive_ids, adaptive_state = _adaptive_embedding_fusion(
                ...
                enable_llm_recall_control=bool(getattr(args, "agent3_adaptive_llm_recall_control", False)),
                llm_model_name=str(getattr(args, "text_model", "Qwen/Qwen3-8B")),
                enable_llm_pseudo_query_rewrite=bool(getattr(args, "agent3_pseudo_query_llm_rewrite", False)),
                user_query=query,
            )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--agent3-adaptive-min-total-recall", type=int, default=500, help="Agent3 text+vl融合召回总量下限（受候选池大小约束）。")
    parser.add_argument("--agent3-adaptive-max-total-recall", type=int, default=500, help="Agent3 text+vl融合召回总量上限（受候选池大小约束）。")
    parser.add_argument("--agent3-adaptive-max-pseudo-queries", type=int, default=8, help="Agent3每次最多使用多少历史商品构造伪查询。")
    parser.add_argument("--agent3-adaptive-llm-recall-control", action="store_true", help="开启后由LLM直接决策total_recall（仍受min/max边界约束）。")
    parser.add_argument("--agent3-pseudo-query-llm-rewrite", action="store_true", help="开启后由LLM直接重写历史item的pseudo query。")
