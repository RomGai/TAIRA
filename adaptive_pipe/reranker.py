...
class LLMItemReranker:
    ...
    def _build_scoring_prompt(
        ...
        return (
            ...
            "评分规则（只能取1~5）：\n"
            "1 = 明确触碰 Must_Avoid，或与用户 Query 的核心意图明显冲突，即使满足部分偏好也不可推荐。\n"
            "2 = 与 Query 仅弱相关，只命中少量表面关键词或边缘属性，未有效满足用户当前核心诉求。\n"
            "3 = 基本相关，能满足 Query 的部分核心诉求，或满足若干 Must_Have / Nice_to_Have，但仍存在缺口。\n"
            "4 = 高相关，较好满足 Query 的核心诉求，并覆盖大多数 Must_Have，同时有一定 Nice_to_Have 加分，无明显冲突。\n"
            "5 = 强匹配，完整满足 Query 的核心诉求和 Must_Have，无 Must_Avoid 冲突，并且在 Nice_to_Have 或用户潜在偏好上表现突出。\n\n"
            "判断优先级：\n"
            "1. 先判断候选商品是否满足用户 Query 的核心购买意图。\n"
            "2. 再判断是否覆盖 Must_Have。\n"
            "3. 再参考 Nice_to_Have、Predicted_Next_Items 与候选商品画像的契合度。\n"
            "4. 若 Query 核心诉求不满足，即使 Must_Have 或 Nice_to_Have 命中较多，也不应给高分。\n"
            "请只输出一个数字：1/2/3/4/5。\n\n"
            f"用户Query: {query}\n"
