...
def _fixed_modal_fusion_recall_ids(
    all_item_ids: List[str],
    text_rank_indices: np.ndarray,
    vl_rank_indices: np.ndarray | None,
    recall_size: int,
    text_ratio: float,
) -> Tuple[List[str], Dict[str, Any]]:
    total = int(max(1, min(500, recall_size)))
    text_w = float(min(1.0, max(0.0, text_ratio)))
    vl_w = float(1.0 - text_w)

    text_k = int(round(total * text_w))
    text_k = min(total, max(0, text_k))
    vl_k = total - text_k

    out: List[str] = []
    for idx in text_rank_indices[:text_k]:
        iid = all_item_ids[int(idx)]
        if iid not in out:
            out.append(iid)

    if vl_rank_indices is not None and vl_k > 0:
        for idx in vl_rank_indices:
            iid = all_item_ids[int(idx)]
            if iid not in out:
                out.append(iid)
            if len(out) >= text_k + vl_k:
                break

    if len(out) < total:
        for idx in text_rank_indices:
            iid = all_item_ids[int(idx)]
            if iid not in out:
                out.append(iid)
            if len(out) >= total:
    ...
    state = {
        "enabled": True,
        "mode": "fixed",
        "total_recall": int(total),
        "text_weight": float(text_w),
        "vl_weight": float(vl_w),
        "text_recall_size": int(text_k),
        "vl_recall_size": int(total - text_k),
        "final_modal_ratio": f"{int(round(text_w * 100))}:{int(round(vl_w * 100))}",
    }
...
def run(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    for row_idx, row in query_df.iterrows():
        ...
        if bool(getattr(args, "enable_agent3_fixed_modulation", False)):
            fixed_ids, fixed_state = _fixed_modal_fusion_recall_ids(
                all_item_ids=all_item_ids,
                text_rank_indices=full_rank_indices,
                vl_rank_indices=qwen3vl_rank_indices_all,
                recall_size=int(getattr(args, "agent3_fixed_recall_size", 500)),
                text_ratio=float(getattr(args, "agent3_fixed_text_recall_ratio", 0.5)),
            )
            top_ids = _merge_unique_ids(top_ids, fixed_ids)
            used_k = len(top_ids)
            kw_debug["adaptive_embedding_state"] = fixed_state
        elif bool(getattr(args, "enable_agent3_adaptive_weighting", False)):
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--enable-agent3-fixed-modulation", action="store_true", help="开启固定modal参数融合召回（固定recall_size+文本召回比例）。")
    parser.add_argument("--agent3-fixed-recall-size", type=int, default=500, help="固定融合召回总量（<=500）。")
    parser.add_argument("--agent3-fixed-text-recall-ratio", type=float, default=0.5, help="固定文本召回比例，VL比例=1-该值。")
    parser.add_argument(
